...
final class EditorDocument: NSDocument {
    ...
    var canUndo: Bool {
        get async {
            ...
            return await (try? bridge?.history.canUndo()) ?? false
        }
    }
    ...
    var canRedo: Bool {
        get async {
            ...
            return await (try? bridge?.history.canRedo()) ?? false
        }
    }
    ...
    private var suggestedFileExtension: NewFilenameExtension?
    private weak var hostViewController: EditorViewController?

    /// 当标题（suggestedFilename）发生变化时发送的通知
    ...
    private var suggestedFilename: String? {
        didSet {
            guard suggestedFilename != oldValue else { return }
            NotificationCenter.default.post(name: Self.suggestedFilenameDidChangeNotification, object: self)
        }
    }
    ...
    /// 为嵌入模式设置 host view controller
    /// - Parameter controller: 关联的 EditorViewController
    func setHostViewController(_ controller: EditorViewController) {
        hostViewController = controller
        // 如果文件已经读取完成，立即更新 representedObject
    ...
    override func makeWindowControllers() {
        ...
        guard
            let windowController = storyboard
                .instantiateController(withIdentifier: sceneIdentifier) as? EditorWindowController else {
            return
        }
...
extension EditorDocument {
    ...
    override var displayName: String? {
        get {
            ...
            if fileURL == nil {
                if let newFileName = suggestedFilename ?? externalFilename {
                    return newFileName
                }
                return Localized.Document.untitled
            }
    ...
    override func writableTypes(for saveOperation: NSDocument.SaveOperationType) -> [String] {
        // 使用自定义 UTType 标识符，确保每个扩展名有独立的类型
        // - app.screenpresenter.md (.md)
        // - app.screenpresenter.markdown (.markdown)
        // - app.screenpresenter.txt (.txt)
        let types = NewFilenameExtension.allCases.map(\.exportedType)
        
        // Enable *.textbundle only when we have the bundle, typically for a duplicated draft
        return textBundle == nil ? types : ["org.textbundle.package"] + types
    }
    ...
    override func fileNameExtension(forType typeName: String, saveOperation: NSDocument.SaveOperationType) -> String? {
        ...
        // 优先使用用户在保存面板中选择的扩展名
        if let suggested = suggestedFileExtension {
            return suggested.rawValue
        }
    ...
    override func prepareSavePanel(_ savePanel: NSSavePanel) -> Bool {
        ...
        if textBundle == nil {
            savePanel.accessoryView = EditorSaveOptionsView.wrapper(for: .savePanel) { [
                weak self,
                weak savePanel
            ] result in
                switch result {
        ...
        // 首次保存时，优先使用文档标题作为默认文件名，无标题则回退到本地化的 "未命名"
...
extension EditorDocument {
    ...
    override func save(
        ...
        let resolvedURL = resolvedSaveURL(for: url, typeName: typeName, saveOperation: saveOperation)
        super.save(to: resolvedURL, ofType: typeName, for: saveOperation, completionHandler: completionHandler)
    }
...
private extension EditorDocument {
    ...
    func resolvedSaveURL(for url: URL, typeName: String, saveOperation: NSDocument.SaveOperationType) -> URL {
        guard url.pathExtension.isEmpty else { return url }

        let fileExtension = if typeName.isEmpty {
            AppPreferences.General.newFilenameExtension.rawValue
        } else {
            fileNameExtension(forType: typeName, saveOperation: saveOperation)
            ?? AppPreferences.General.newFilenameExtension.rawValue
        }
...
private extension EditorDocument {
    ...
    func updateContent(userInitiated: Bool = false) async {
        ...
        if insertFinalNewline || trimTrailingWhitespace {
            formatCompleted = await (try? bridge?.format.formatContent(
