...
final class EditorViewController: NSViewController {
    /// 预览模式变化通知
    static let previewModeDidChangeNotification = Notification.Name("EditorViewControllerPreviewModeDidChange")
    ...
    /// 嵌入模式下的格式工具栏
    private(set) lazy var formatBar: EditorFormatBar? = {
        let bar = EditorFormatBar()
        bar.delegate = self
        return bar
    }()

    /// Markdown 预览视图（嵌入模式下可用）
    var previewView: MarkdownPreviewView?
    var previewThemeMode: MarkdownEditorThemeMode = .system

    /// 是否将主题应用到窗口级别（appearance、backgroundColor）
    /// 嵌入模式下应设为 false，避免编辑器主题影响宿主窗口
    var appliesThemeToWindow: Bool = true

    /// 宿主是否处于全屏模式（由 MarkdownEditorView 设置）
    /// 全屏时跳过顶部 safeAreaInsets 补偿，使内容延伸到顶部
    var isHostFullScreen: Bool = false {
        didSet {
            guard oldValue != isHostFullScreen else { return }
    ...
    private(set) lazy var webView: WKWebView = {
        ...
        // Respect user settings for Writing Tools behavior (macOS 15+)
        // Note: writingToolsBehavior requires macOS 15+ SDK at compile time,
        // disabled for macOS 14 compatibility
        ...
        if #available(macOS 15.1, *) {
            writingToolsObservation = webView.observe(\.isWritingToolsActive) { [weak self] _, _ in
                ...
                updateWritingTools(isActive: self.webView.isWritingToolsActive)
            }
        }
    ...
    private(set) lazy var completionContext = TextCompletionContext(
...
extension EditorViewController {
    ...
    func currentZoomLevel() -> Double {
        if isPreviewMode {
            return previewView?.zoomLevel ?? 1.0
        }
    ...
    func canZoomIn() -> Bool {
        if isPreviewMode {
            return previewView?.canZoomIn() ?? false
        }
        return webView.magnification < 3.0
    }

    func canZoomOut() -> Bool {
        if isPreviewMode {
            return previewView?.canZoomOut() ?? false
        }
        return webView.magnification > 1.0
    }
    ...
    func zoomIn() {
        if isPreviewMode {
            ...
            webView.magnification = min(3.0, webView.magnification + 0.1)
        }
    }
    ...
    func zoomOut() {
        if isPreviewMode {
            ...
            webView.magnification = max(1.0, webView.magnification - 0.1)
        }
    }

    func setPreviewThemeMode(_ mode: MarkdownEditorThemeMode) {
        previewThemeMode = mode
        previewView?.setThemeMode(mode)
    }

    // MARK: - Private

    /// Configure Writing Tools behavior for macOS 15+
    /// Disabled for macOS 14 deployment target compatibility
    private func configureWritingToolsBehavior(_ config: WKWebViewConfiguration) {
        // Writing Tools requires macOS 15.1+ API which is not available
        // when building with macOS 14 deployment target.
        // This feature is intentionally disabled for ScreenPresenter integration.
    }
}
