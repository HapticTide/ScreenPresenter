...
extension EditorViewController {
    ...
    func configureToolbar() {
        guard appliesThemeToWindow else {
            // 嵌入模式：不创建工具栏，不修改窗口属性
            return
        }
    ...
    func updateWritingTools(isActive: Bool) {
        let performUpdate = {
            ...
            self.bridge.writingTools.setActive(
                isActive: isActive,
                reselect: MarkdownWritingTools.shouldReselect(with: MarkdownWritingTools.requestedTool)
            )
    ...
    func layoutPanels(animated: Bool = false) {
        ...
        if AppDesign.modernTitleBar {
            modernEffectHeight.constant = isHostFullScreen ? 0 : view.safeAreaInsets.top
    ...
    /// 布局嵌入模式下的格式工具栏
    func layoutFormatBar() {
        guard let bar = formatBar else { return }
        let shouldShow = shouldShowEmbeddedFormatBar
        bar.isHidden = !shouldShow
        guard shouldShow else { return }
        bar.frame = CGRect(
            x: 0,
            y: view.bounds.height - EditorFormatBar.height,
            width: view.bounds.width,
            height: EditorFormatBar.height
        )
    }
...
private extension EditorViewController {
    ...
    var contentHeight: Double {
        ...
        // 宿主全屏时跳过 safeAreaInsets，使编辑器内容延伸到顶部
        if !isHostFullScreen {
            height -= view.safeAreaInsets.top
        }
        // 嵌入模式下为格式工具栏留出空间
        if shouldShowEmbeddedFormatBar {
            height -= EditorFormatBar.height
        }
        return height
    }

    var shouldShowEmbeddedFormatBar: Bool {
        guard formatBar != nil else { return false }
    ...
    var findPanelHeight: Double {
        ...
        case .hidden: 0
        case .find: findPanel.frame.height
        case .replace: findPanel.frame.height + replacePanel.frame.height
        }
    }
