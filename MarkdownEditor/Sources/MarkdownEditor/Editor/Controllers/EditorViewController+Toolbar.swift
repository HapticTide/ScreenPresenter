...
extension EditorViewController {
    ...
    func showTableOfContentsMenu() {
        ...
        if
            let menu = (tableOfContentsItem as? NSMenuToolbarItem)?.menu,
...
extension EditorViewController: NSToolbarDelegate {
    ...
    func toolbarAllowedItemIdentifiers(_ toolbar: NSToolbar) -> [NSToolbarItem.Identifier] {
        NSToolbarItem.Identifier.allItems + AppRuntimeConfig.customToolbarItems.map(\.identifier)
    }
}
...
private extension EditorViewController {
    ...
    var shareDocumentItem: NSToolbarItem {
        ...
        item.image = NSImage(
            systemSymbolName: Icons.squareAndArrowUp,
            accessibilityDescription: Localized.Toolbar.shareDocument
        )
        item.delegate = self
        return item
    }
    ...
    var writingToolsItem: NSToolbarItem? {
        if #available(macOS 15.1, *), let menu = systemWritingToolsMenu {
            .with(identifier: .writingTools, menu: menu.copiedMenu)
        } else {
            nil
        }
    }
    ...
    func updateTableOfContentsMenu(_ menu: NSMenu) {
        ...
        Task {
            ...
            let baseLevel = tableOfContents?.map(\.level).min() ?? 1
