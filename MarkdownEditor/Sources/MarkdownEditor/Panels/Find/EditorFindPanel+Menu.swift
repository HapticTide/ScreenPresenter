...
extension EditorFindPanel {
    ...
    func resetMenu() {
        ...
        menu.addItem(withTitle: Localized.Search.replace, action: #selector(enableReplaceMode(_:)))
            .setOn(mode == .replace)
        menu.addItem(.separator())

        let caseItem = menu.addItem(
            withTitle: Localized.Search.caseSensitive,
        ...
        let diacriticItem = menu.addItem(
            withTitle: Localized.Search.diacriticInsensitive,
            action: #selector(toggleDiacriticInsensitive(_:))
        )
        ...
        let literalItem = menu.addItem(
            withTitle: Localized.Search.literalSearch,
            action: #selector(toggleLiteralSearch(_:))
        )
        literalItem.tag = Option.literalSearch.rawValue
        literalItem.setOn(AppPreferences.Search.literalSearch)

        let regexItem = menu.addItem(
            withTitle: Localized.Search.regularExpression,
            action: #selector(toggleRegularExpression(_:))
        )
...
private extension EditorFindPanel {
    ...
    func updateIconTintColor() {
        let shouldTint = (searchField.searchMenuTemplate?.items.filter {
            Option.allCases.map(\.rawValue).contains($0.tag)
