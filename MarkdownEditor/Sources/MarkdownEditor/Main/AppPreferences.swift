...
enum Appearance: Codable {
    ...
    func resolved(with appearance: NSAppearance = NSApp.effectiveAppearance) -> NSAppearance? {
        ...
        case .system:
            nil
        case .light:
            NSAppearance(named: appearance.resolvedName(isDarkMode: false))
        case .dark:
            NSAppearance(named: appearance.resolvedName(isDarkMode: true))
        }
    }
}
...
enum NewFilenameExtension: String, Codable, CaseIterable {
    ...
    /// 导出类型标识符，用于 Info.plist 的 UTExportedTypeDeclarations
    ///
    /// 使用自定义标识符（如 app.screenpresenter.*）避免系统关联冲突
    var exportedType: String {
        "app.screenpresenter.\(rawValue)"
    }

    /// 对应的 UTType
    var uniformType: UTType {
        UTType(exportedType) ?? .plainText
    }
    ...
    static func preferredExtension(for typeName: String) -> Self {
        // 首先尝试按导出类型匹配
        if let match = allCases.first(where: { $0.exportedType == typeName }) {
            return match
        }
        // 兼容旧的标准 UTI 匹配
        if typeName == "net.daringfireball.markdown" {
            return AppPreferences.General.newFilenameExtension == .markdown ? .markdown : .md
        }
        if typeName == "public.plain-text" {
            return .txt
        }
        // 按扩展名尝试匹配
        return allCases.first { $0.rawValue == typeName } ?? AppPreferences.General.newFilenameExtension
    }
}
