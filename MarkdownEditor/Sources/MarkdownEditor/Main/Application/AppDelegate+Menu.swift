...
private extension AppDelegate {
    func reconfigureMainFileMenu(document: EditorDocument?) {
        for item in [openFileInMenu, reopenFileMenu, lineEndingsMenu] {
            item?.superMenuItem?.isEnabled = document?.fileURL != nil
        }
    ...
    func reconfigureReopenFileMenu(document: EditorDocument?) {
        ...
        for encoding in EditorTextEncoding.allCases {
            let item = reopenFileMenu?.addItem(
                withTitle: encoding.description,
                action: #selector(EditorViewController.reopenWithEncoding(_:))
            )
